...
const byte serialEn = 3;    // Connect UART output enable of LSA08 to pin 2
const byte junctionPulse = 4;  // Pin connected to Junction Pulse (Input)
...
void loop() {
  ...
  if (Jpulse == 1) {  // If a junction pulse is detected
    ...
    turnRight();  // Turn right using MPU6050 sensor
  }
...
// Function to perform a right turn using MPU6050
void turnRightWithMPU() {
  ...
  if (!hasTurned) {
    ...
    // Calculate the angle difference to see if the robot turned 90 degrees
    float angleTurned = currentYaw - initialYaw;

    // If the angle difference is less than 90 degrees, keep turning right
    while (abs(angleTurned) < targetAngle) {
      turnRight();  // Perform the right turn
      mpu.update();  // Continuously update MPU6050 data
      currentYaw = mpu.getAngleZ();  // Update current yaw
      angleTurned = currentYaw - initialYaw;  // Calculate angle turned
...
// Function to perform a right turn (adjust motor control as needed)
void turnRight() {
  digitalWrite(dir1, LOW);   // Motor A backward
