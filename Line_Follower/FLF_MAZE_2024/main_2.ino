...
const int uturnDuration = 230; //Duration for uturn
const int overshootDuration = 200; //duration of overshooting once turn has been noticed

//in reference to moveStraight() function
const int strcheckDuration = 150; //Moves forward a bit to check if straight path exists
const int endcheckDuration = 150; //Moves forwrd to determine the end of the maze (white solid box)
const int concheckDuration = 100; //If straight path exists
...
float initialYaw = 0;
float targetAngle = 80;//90
bool hasTurned = false;
const int TURN_SPEED = 75;//75
...
void dry_run()
{
    ...
    if (rightPrio)
    {
        if (Jpulse == 1 && !executingTurn)
        {
            ...
            {
                // Right path detected
                moveStraight(overshootDuration);
                digitalWrite(serialEn, LOW);
                while (Serial.available() <= 0)
                    ;
            ...
            else
            {
                ...
                moveStraight(strcheckDuration); // Move forward a bit
                digitalWrite(serialEn, LOW);
                while (Serial.available() <= 0)
                    ;
                ...
                if (straightCheck != 255)
                {
                    ...
                    moveStraight(concheckDuration); // Continue forward
                    digitalWrite(serialEn, LOW);
                    while (Serial.available() <= 0)
                        ;
                    positionVal = Serial.read();
